...
**Follow-up — quizzes, dummy content (added 2026-07-15, not in the original checklist):**

- New `quizzes` table (`org_id`, `course_id`, `title`, `questions jsonb`, `status draft/published`) — same RLS shape as courses/lessons (`is_org_content_manager` writes, org-member reads). `questions` is a single `jsonb` array column rather than a child table — no per-question CRUD is needed yet, so a join table would be pure ceremony (revisit if real question editing lands later).
- Three new `mcp_*` RPCs (`mcp_create_quiz`, `mcp_list_quizzes`, `mcp_delete_quiz`), same `SECURITY DEFINER` / `mcp_authenticated_org(p_key_hash)` pattern as M3. `mcp_create_quiz` defaults the title to `"<course title> Quiz"` server-side (looked up from `courses` in the same statement that validates the course belongs to the caller's org) when the caller omits one.
- New MCP tool `generate_quiz` (course_id, optional title/status) — content is deliberately **dummy placeholder questions** (`src/lib/quiz-dummy.ts`, shared by both the MCP tool and the web "Generate quiz" button so the two surfaces can't drift): 5 fixed placeholder Q&A pairs per quiz. Real AI-authored question generation is a future upgrade, out of scope here. Plus `list_quizzes` / `delete_quiz`, same shape as the courses module's read/delete tools.
- The two course-creation tools (`create_course`, `create_course_from_youtube_playlist`) now steer the calling AI on quizzes the same way M4 already steers it on funnels: `create_course`'s `description` tells the AI to call `generate_quiz` right after creation *if the user explicitly asked for a quiz*, and a new `quizSuggestionBlock(courseTitle)` nudge (mirroring `sellSuggestionBlock`) is appended to both tools' response text so the AI offers a quiz instead when the user didn't ask — never generating one unprompted.
- Web UI: course detail page (`/dashboard/courses/[id]`) gets a third card, `QuizzesManager` (`src/components/courses/quizzes-manager.tsx`) — list of quizzes with question count + status badge, a "Generate quiz" button (no dialog/fields — it's dummy content, nothing to fill in) and inline delete, gated by the same `canManage` flag as lessons. Backed by `src/lib/actions/quizzes.ts` (`createQuiz`/`deleteQuiz`, direct Supabase writes through RLS, same pattern as `lessons.ts`) and `getQuizzes` in `dal.ts`.
- **Verified live (against the real project):** migrations applied (`create_quizzes_schema`, `create_quizzes_mcp_rpcs`); `get_advisors({type:"security"})` shows only the same accepted `anon`/`authenticated`-callable `mcp_*` warning class as M1–M5, nothing new, no RLS-disabled warning on `quizzes`. End-to-end over the real `/api/mcp`: `signup` → `create_course` (response correctly carries the quiz-suggestion nudge alongside the funnel/automation one) → `generate_quiz` (no title given → correctly defaulted to `"Intro to Baking Quiz"`, 5 placeholder questions) → `list_quizzes` → `delete_quiz` → re-`delete_quiz` on the same id → clean tool error (`isError: true`, "quiz not found in this organization"), not a crash → `list_quizzes` confirms 0 remain. RLS cross-check via direct PostgREST calls: signed-in real owner sees exactly their quiz; `anon` sees an empty result (not an error); `anon` direct insert into `quizzes` → `42501` rejected, confirming the RPC/RLS path is the only write path. `tsc --noEmit` / `eslint` clean on all new/changed files.
- **Not independently re-verified via an actual browser**: no browser-automation tool was available in this session (same caveat as M5) — the new page section follows the exact same Server Component + `dal.ts` + client-action pattern already proven to render correctly for lessons in M3, and the underlying RLS-scoped query was verified directly instead.
- Test org/course/quiz/api key cleaned from `public` schema after verification. Test `auth.users` row left (harmless `+creators.skillguy.invalid` placeholder from the MCP `signup` call), same pattern as M1–M5.

**Follow-up — student-facing quiz view (added 2026-07-15):**

- New public RLS policy `"anyone can view quizzes of published courses"` on `quizzes` — mirrors the lessons policy from M3 (`exists (select 1 from courses where courses.id = quizzes.course_id and courses.status = 'published')`), plus its own `status = 'published'` check since (unlike lessons) quizzes carry their own draft/published state independent of the parent course.
- `getPublicQuizzes(courseId)` / `getPublicQuiz(courseId, quizId)` added to `src/lib/public-courses.ts`, same shape as `getPublicLessons` — published-only, `.eq("status", "published")` as defense-in-depth alongside RLS.
- Student course page (`/[org]/courses/[course]`) gets a fourth tab, "Quiz", listing published quizzes with question counts, linking to a new public route `/[org]/courses/[course]/quiz/[quiz]`. That page renders `QuizTaker` (`src/components/courses/quiz-taker.tsx`) — a client component for picking answers and seeing a score on submit. **No grading backend or attempt persistence** — scoring is client-side only against `correct_answer` shipped in the page payload, same "demo, not the real thing" simplification precedent as M4's checkout (no student-accounts/progress-tracking table exists yet — that's M8+ territory).
- Since quizzes are created `draft` by default (via both `generate_quiz` and the dashboard's "Generate quiz" button) and there was previously no way to flip that, added a **Publish/Unpublish toggle** per quiz row in `QuizzesManager` (`src/lib/actions/quizzes.ts`'s new `setQuizStatus`, direct Supabase write through the existing `is_org_content_manager` update policy — no new RPC needed). No new MCP publish/unpublish tool added — `generate_quiz` already accepts `status: "published"` at creation time, which covers the MCP path.
- **Verified live (against the real project):** migration `public_read_published_quizzes` applied; `get_advisors({type:"security"})` shows only the same accepted warning classes as before, no new RLS-disabled warning. End-to-end: created a published course via MCP, generated one `published` quiz and one `draft` quiz on it; real HTTP GET on the course page returned 200 with the published quiz's title/link present and the draft quiz's title absent; the quiz-taking page returned 200 with real placeholder question text; a nonexistent quiz id under a real course → 404; a nonexistent org slug → 404 (org-scoping, not just existence). `tsc --noEmit` / `eslint` clean on all new/changed files.
- **Not independently re-verified via an actual browser** (no interactive click-through of the answer-selection/scoring UI) — same browser-automation-unavailable caveat as the M5/quiz-dummy-content follow-ups above; the HTML response was verified directly and `QuizTaker`'s scoring logic is plain client-side state, not touching any new backend surface.
- Test org/course/quizzes/api key cleaned from `public` schema after verification. Same harmless test `auth.users` row pattern as before.
